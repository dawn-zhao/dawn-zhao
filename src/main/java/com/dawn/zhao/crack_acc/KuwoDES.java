...
public final class KuwoDES {
    ...
    public synchronized static byte[] encrypt(byte[] src, int srcLength, byte[] key, int keyLength) {
        ...
        System.out.println(JSONObject.toJSONString(pEncyrpt));
    ...
    public static void main(String[] args) {
        byte[] bytes = new byte[(int)Math.pow(2, 8)];
        byte b = -128;
        for (int i = 0; i < bytes.length; i++,b++) {
            bytes[i] = b;
            System.out.println(b + ":" + new String(new byte[]{b}));
        }
        System.out.println(new String(Base64Coder.encode(bytes, bytes.length)));
    }
